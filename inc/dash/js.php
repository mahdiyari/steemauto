	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			if(this.responseText == 1){
				$.notify({
					icon: 'pe-7s-check',
					message: "followed with 100% upvote weight! You can access this setting after reloading the page."
				},{
					type: 'success',
					timer: 6000
				});
...
function post(){
	...
	var rewardstype = encodeURIComponent(document.getElementById('rewardstype').value);
	var upvotepost = 1;
	...
	if(tagss.length > 5){
		...
		var arrtags = '';
		for(i in tagss){
			arrtags = arrtags + 'tags[]='+encodeURIComponent(tagss[i])+'&';
		}
		var xmlhttp = new XMLHttpRequest();
		xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
		...
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send("date="+date+"&title="+title+"&content="+content+'&rewardstype='+rewardstype+'&upvotepost='+upvotepost+'&'+arrtags);
	}
